#version 330 core
//-----------------------------------//
//此為輸入光源為Spotlight的像素著色器//
//-----------------------------------//
/*
Spotlight為指向某一方向且受角度限制的光
源，通常用來照明特定的區域，例如:手電筒。
*/
...
struct Light//光源的屬性
{
    ...
    float cutOff;//光源開始淡化的角度的cos值
    float outerCutOff;//光源邊界的角度cos值
...
void main()
{
    ...
    //受到衰減率影響的環境光、漫射光、反射光的顏色
    ambient  *= attenuation; 
    diffuse  *= attenuation;
    specular *= attenuation;
    
    //此像素到光源的單位方向向量與光源照射方向的單位反向量的角度的cos值
    float theta     = dot(lightDir, normalize(-light.direction));
    //角度衰減率 = (theta - light.outerCutOff) / epsilon
    //角度衰減率函數的分母
    float epsilon   = light.cutOff - light.outerCutOff;
    //光線強度
    float intensity = clamp((theta - light.outerCutOff) / epsilon, 0.0, 1.0);   
    
    //受到角度衰減率影響的環境光、漫射光、反射光的顏色
    //ambient  *= intensity;//保留環境光的強度
    diffuse  *= intensity;
    specular *= intensity;
